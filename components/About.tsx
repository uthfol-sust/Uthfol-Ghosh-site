...
export default function About() {
  ...
  return (
    <section id="about" className="relative overflow-hidden px-6 pb-8 pt-0 sm:px-8 lg:px-10 lg:pb-10 lg:-mt-4">
      ...
      <div className="relative mx-auto mt-15 max-w-7xl">
        <SectionTitle
          eyebrow="Who I'm"
        ...
        <div className="mt-14 grid items-start gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-14">
          ...
          >
            <div>
              <div className="flex border-b border-white/15 px-2">
                {tabs.map((tab) => {
                  const Icon = tab.icon;
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-5 px-7 py-3 text-xs cursor-pointer font-medium transition-colors duration-200 whitespace-pre-line ${
                        isActive
                          ? "text-cyan-300 text-xs sm:text-xl"
                  ...
                  <AnimatePresence mode="wait">
                    {activeTab === "education" && (
                      <motion.div
                        ...
                        className="pl-3"
                      >
                        <div className="rounded-xl border border-white/[0.2] bg-white/[0.01] p-4">
                          <div className="flex items-start justify-between gap-4">
                            <div className="min-w-0">
                              <p className="text-sm font-semibold text-white sm:text-base">
                                {education.degree}
                              </p>
                              <p className="mt-1 text-sm text-zinc-400">{education.institution}, Sylhet</p>
                            </div>
                        ...
                        <div className="space-y-2">
                          {skillGroups.map((group) => {
                            ...
                            return (
                              <div
                                key={group.category}
                                className="flex items-center gap-3 rounded-[10px] border border-white/[0.08] bg-white/[0.01] px-3 py-3"
                              >
                    ...
                    {activeTab === "soft" && (
                      <motion.div
                        ...
                        className="pl-3"
                      >
                        <div className="divide-y divide-white/[0.2] rounded-[10px] border border-white/[0.08] bg-white/[0.01]">
                          {softSkills.map((item) => {
          ...
          >
            ...
            <div className="relative mt-2">
              <p className="flex items-center gap-2 pl-6 text-lg leading-9 text-zinc-200 sm:text-xl">
                <Sparkles className="h-5 w-5 text-cyan-300/80" />
                <span className="font-medium text-cyan-300">{"Working On"}</span>
              </p>
              <div className="ml-6 mt-1 h-0.5 w-35 bg-white/10" />
            </div>
            ...
            <div className="mt-5 inline-flex max-w-full items-center gap-2.5 rounded-full border border-white/[0.06] bg-white/[0.02] px-3.5 py-1.5">
              <span className="flex h-1.5 w-1.5 items-center justify-center">
                ...
                <span className="absolute h-2.5 w-2.5 animate-ping rounded-full bg-cyan-300/30" />
              </span>
              <span className="shrink-0 font-mono text-[14px] uppercase tracking-[0.15em] text-zinc-500">
                Currently:
              </span>
              <AnimatePresence mode="wait">
                <motion.span
...
function SectionTitle({
  ...
  return (
    <div>
      <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-[14px] font-semibold uppercase tracking-[0.3em] text-cyan-200/70 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
        <span className="flex h-1.5 w-1.5 items-center justify-center">
      ...
      <div ref={descRef} className="mt-4 relative pl-6">
        <div
          className="absolute left-0 top-0 w-0.5"
          style={{
            height: descHeight || "100%",
            background: "linear-gradient(to bottom, transparent 0%, rgba(34,211,238,1) 40%, rgba(34,211,238,1) 80%, transparent 100%)",
            //backgroundSize: "100% 400%",
            // animation: descHeight ? "bright-move 4s ease-in-out infinite" : "none",
          }}
        />
        <p className="text-base leading-8 text-zinc-400 sm:text-1xl">{description}</p>
      </div>
    </div>
  );
}
